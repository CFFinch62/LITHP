...
# Change to the script's directory so build artifacts go to LITHP/
os.chdir(SCRIPT_DIR)
...
def get_args() -> list[str]:
    ...
    if system == "Darwin":
        ...
        args += ["--icon", "images/lithp_icon.ico"]
...
def build() -> None:
    ...
    try:
        ...
        # Copy newly built executable to the main LITHP directory for ease of access
        system = platform.system()
        executable_name = APP_NAME + (".exe" if system == "Windows" else "")
        dist_path = os.path.join("dist", APP_NAME, executable_name)
        if os.path.exists(dist_path):
            shutil.copy2(dist_path, executable_name)
